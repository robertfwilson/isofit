...
'''
NOTE: ALL FUNCTIONS IN THIS FILE ARE TAKEN FROM THE ISOCHRONES PYTHON PACKAGE:
https://github.com/timothydmorton/isochrones/blob/9fd7b6eb0259187a19d992b8d045655cf6f3d371/isochrones/interp.py

ALL CREDIT SHOULD GO TO THE ORIGINAL AUTHOR, TIMOTHY MORTON. 
PLEASE CITE: https://ui.adsabs.harvard.edu/abs/2015ascl.soft03010M/abstract
'''
    ...
    def _make_grid(self, df, recalc=False):
        ...
        if self.filename is not None and os.path.exists(self.filename) and not recalc:
            ...
            if not self.is_full:  # Need to make a full grid and pad with nans
                ...
                grid_df.loc[df.index] = df.values
                print(len(idx) )
